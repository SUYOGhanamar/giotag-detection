...
// =============================================================================
//  GEODETECT v2  ·  Deepfake Geo-Tag Detection
//  Paste into lib/main.dart
//  Packages used: google_fonts, image_picker, fl_chart,
//                 flutter_map, latlong2, shared_preferences, intl, uuid
//  NO flutter_animate / percent_indicator / dotted_border / timeline_tile
// =============================================================================
...
class AppState extends ChangeNotifier {
  ...
  void setResult(AnalysisResult r) { lastResult = r; tabIndex = 1; notifyListeners(); }
        ...
        builder: (_, __) => Stack(
          children: [
            ...
            Center(
              child: Column(
                ...
                children: [
                    ...
                    child: RichText(
                      text: TextSpan(
                        style: GoogleFonts.spaceGrotesk(
                            ...
                            letterSpacing: -2),
                        children: [
                          const TextSpan(text: 'GEO', style: TextStyle(color: C.t1)),
                          TextSpan(text: 'DETECT',
                              style: TextStyle(
                                foreground: Paint()
                  ...
                  Opacity(
                    opacity: _fade.value,
                    child: Text('DEEPFAKE · GEO-TAG · FORENSICS',
                        style: GoogleFonts.sourceCodePro(
                            fontSize: 11, letterSpacing: 3, color: C.t3)),
                  ),
            ...
            Positioned(
              ...
              child: Opacity(
                opacity: _fade.value,
                child: Text('v2.0 · AI-Powered Detection',
    ...
    return Stack(alignment: Alignment.center, children: [
      Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: C.cyan.withOpacity(0.2), blurRadius: 40, spreadRadius: 8)],
        ),
      ),
      Container(
        width: 76, height: 76,
        decoration: BoxDecoration(
          ...
          border: Border.all(color: C.cyan.withOpacity(0.45), width: 1.5),
        ),
        child: const Icon(Icons.radar_rounded, color: C.cyan, size: 36),
      ),
    ]);
  }
}
    ...
    return ListenableBuilder(
      ...
      builder: (_, __) {
        final screens = [
          ...
          ResultsScreen(result: appState.lastResult),
          GeoMapScreen(result: appState.lastResult),
          const HistoryScreen(),
        ...
        return Scaffold(
          ...
          bottomNavigationBar: Container(
            ...
            child: BottomNavigationBar(
              ...
              elevation: 0,
              items: const [
                BottomNavigationBarItem(icon: Icon(Icons.upload_file_outlined),   activeIcon: Icon(Icons.upload_file_rounded),   label: 'Upload'),
                BottomNavigationBarItem(icon: Icon(Icons.analytics_outlined),      activeIcon: Icon(Icons.analytics_rounded),     label: 'Results'),
                BottomNavigationBarItem(icon: Icon(Icons.map_outlined),            activeIcon: Icon(Icons.map_rounded),           label: 'Geo Map'),
...
class _UploadState extends State<UploadScreen> with TickerProviderStateMixin {
    ...
    return ListenableBuilder(
      ...
      builder: (_, __) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            title: Row(children: [
              const Icon(Icons.radar_rounded, color: C.cyan, size: 20),
              const SizedBox(width: 8),
              Text('GeoDetect', style: GoogleFonts.spaceGrotesk(fontWeight: FontWeight.w900, color: C.t1)),
            ]),
            actions: [_CyanBadge('v2.0'), const SizedBox(width: 16)],
          ),
  ...
  Widget _pipelineGrid() {
    final items = [
      ...
      _PI(Icons.picture_as_pdf_rounded,       'Report Engine',    'Confidence scores + map + PDF export', C.blue),
    ];
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    _IconBtn(Icons.copy_rounded, () {
      Clipboard.setData(ClipboardData(
          text: 'GeoDetect: ${result.authenticityScore}/100 — ${result.status.name}'));
                ...
                child: FlutterMap(
                  ...
                  children: [
                    TileLayer(
                      ...
                      userAgentPackageName: 'com.geodetect.app',
                    ),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _SCard(children: [
              _ApiRow('Google Maps API',   'Landmark & scene verification',    true),
              const Divider(height: 0.5, color: C.border),
              _ApiRow('OpenWeatherMap',    'Historical weather correlation',    true),
              const Divider(height: 0.5, color: C.border),
              _ApiRow('SunCalc API',       'Shadow & sun angle verification',  false),
              const Divider(height: 0.5, color: C.border),
              _ApiRow('Places365 Model',  'Deep scene classification',         true),
            ]),
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Icon(Icons.radar_rounded, color: C.cyan, size: 22),
                  const SizedBox(width: 10),
                  Text('GeoDetect v2.0',
                      style: GoogleFonts.spaceGrotesk(
                ...
                const SizedBox(height: 10),
                const Text(
                  'Advanced deepfake detection for geo-tagged photos. Combines 5-layer visual ML analysis with geo-consistency verification, PRNU sensor fingerprinting, and solar angle cross-checking.',
                  style: TextStyle(fontSize: 12, color: C.t3, height: 1.6),
                ),
                const SizedBox(height: 14),
                Wrap(
                  spacing: 8, runSpacing: 6,
                  children: ['FaceForensics++', 'CLIP', 'FFT Analysis', 'PRNU', 'SunCalc']
                      .map((t) => _SmallChip(t)).toList(),
                ),
              ]),
            ),
